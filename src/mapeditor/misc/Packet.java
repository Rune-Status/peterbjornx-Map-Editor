...
public class Packet {
	...
	public int big4() {
		pos += 4;
		return ((data[pos - 2] & 0xff) << 24) + ((data[pos - 1] & 0xff) << 16) + ((data[pos - 4] & 0xff) << 8)
				+ (data[pos - 3] & 0xff);
	}

	public int biig4() {
		pos += 4;
		return ((data[pos - 3] & 0xff) << 24) + ((data[pos - 4] & 0xff) << 16) + ((data[pos - 1] & 0xff) << 8)
				+ (data[pos - 2] & 0xff);
	}
	...
	public int pos;
	public int bit_pos;
	private static final int[] bitmasks = { 0, 1, 3, 7, 15, 31, 63, 127, 255, 511, 1023, 2047, 4095, 8191, 16383, 32767,
