package mapeditor.jagex.rt3;//Thanks sadistic :3
...
final class BZ2InputStream {
	public static int decompressBuffer(byte outputBuf[], int decompressedSize, byte inputBuf[], int compressedSize, int offset) {
		synchronized (BZ2InputStream.block) {
			BZ2InputStream.block.input = inputBuf;
			BZ2InputStream.block.nextIn = offset;
			BZ2InputStream.block.output = outputBuf;
			BZ2InputStream.block.availOut = 0;
			BZ2InputStream.block.compressedSize = compressedSize;
			BZ2InputStream.block.decompressedSize = decompressedSize;
			...
			decompressedSize -= BZ2InputStream.block.decompressedSize;
	...
	private static void decompress(BZ2Block block) {
		...
		while (reading) {
			byte head = BZ2InputStream.readUChar(block);
			if (head == 23) {
				return;
			}
			/* Magic numbers */
			head = BZ2InputStream.readUChar(block); // 0x41
			head = BZ2InputStream.readUChar(block); // 0x59
			head = BZ2InputStream.readUChar(block); // 0x26
			head = BZ2InputStream.readUChar(block); // 0x53
			head = BZ2InputStream.readUChar(block); // 0x59
			/* CRC checksums */
			head = BZ2InputStream.readUChar(block);
			head = BZ2InputStream.readUChar(block);
			head = BZ2InputStream.readUChar(block);
			head = BZ2InputStream.readUChar(block);
			/* Randomized block, 1 = randomized */
			head = BZ2InputStream.readBit(block);
			...
			block.origPtr = block.origPtr << 8 | head & 0xff;
			for (int i = 0; i < 16; i++) {
				byte used = BZ2InputStream.readBit(block);
			...
			for (int i = 0; i < 16; i++) {
				if (block.inUse16[i]) {
					for (int j = 0; j < 16; j++) {
						byte v = BZ2InputStream.readBit(block);
						if (v == 1) {
			...
			BZ2InputStream.makeMaps(block);
			int alphaSize = block.nInUse + 2;
			int groups = BZ2InputStream.getBits(3, block);
			int selectors = BZ2InputStream.getBits(15, block);
			...
			for (int i = 0; i < groups; i++) {
				int curr = BZ2InputStream.getBits(5, block);
				for (int j = 0; j < alphaSize; j++) {
					do {
						byte flag = BZ2InputStream.readBit(block);
						if (flag == 0) {
							break;
						}
						flag = BZ2InputStream.readBit(block);
						if (flag == 0) {
							curr++;
						} else {
							curr--;
			...
			for (int i = 0; i < groups; i++) {
				...
				BZ2InputStream.createDecodeTables(block.limit[i], block.base[i], block.perm[i], block.len[i], minLen, maxLen, alphaSize);
			...
			int zvec;
			byte bit;
			for (zvec = BZ2InputStream.getBits(zt, block); zvec > tLimit[zt]; zvec = zvec << 1 | bit) {
				zt++;
				bit = BZ2InputStream.readBit(block);
			}
