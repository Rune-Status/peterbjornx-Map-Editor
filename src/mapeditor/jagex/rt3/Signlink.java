		...
		for (int i = threadliveid; threadliveid == i;) {
			if (socketreq != 0) {
				...
				if (midiplay) {
					...
					midiplay = false;
					/*
					 * try { if (musicS != null) { musicSr.stop();
					 * musicSr.close(); } musicS = null; musicSr = null; File
					 * music = new File(midi);
					 * 
					 * if (music.exists()) { musicS =
					 * MidiSystem.getSequence(music); musicSr =
					 * MidiSystem.getSequencer(); musicSr.open();
					 * musicSr.setSequence(musicS); musicSr.start(); } } catch
					 * (Exception ex) { ex.printStackTrace(); } midiplay =
					 * false;
					 */
				}
				savereq = null;
				...
				try {
					System.out.println("urlstream O_o " + urlreq);
