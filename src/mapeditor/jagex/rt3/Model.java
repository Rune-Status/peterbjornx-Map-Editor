...
import mapeditor.jagex.rt4.TriangleNormal;
...
public class Model extends Entity {
    ...
    public static void initialize(int i, OnDemandFetcherParent onDemandFetcherParent) {
      /*
    	modelHeaderCache = new ModelHeader[i];//wtf PETER!!!
  ...
  //  }
    ...
    public static void readHeader(byte abyte0[], int j) {//readheader? - shouldnt realy be being used
		...
		boolean dumpModels = false;
		if (dumpModels) {
			FileOutputStream out = null;
			try {
				out = new FileOutputStream("./dump/models/" + j + ".dat");
			} catch (FileNotFoundException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
			System.out.println("Writing file: "+j+".dat");
			try {
    ...
    public void applyTransform(int frameID) {
        ...
        for (int k = 0; k < animation.stepCount; k++) {
            ...
            transformStep(modelTransform.opcodes[opcodeID], modelTransform.skinList[opcodeID], animation.xOffset[k], animation.yOffset[k], animation.zOffset[k]);
        }
    ...
    public void mixAnimationFrames(int framesFrom2[], int frameId2, int frameId1) {
        ...
        for (int j1 = 0; j1 < animation.stepCount; j1++) {
            ...
            if (k1 != stepIDD || modelTransform.opcodes[k1] == 0)
                transformStep(modelTransform.opcodes[k1], modelTransform.skinList[k1], animation.xOffset[j1], animation.yOffset[j1], animation.zOffset[j1]);
        }
        ...
        for (int l1 = 0; l1 < animation_1.stepCount; l1++) {
            ...
            if (stepID == stepIDD || modelTransform.opcodes[stepID] == 0)
                transformStep(modelTransform.opcodes[stepID], modelTransform.skinList[stepID], animation_1.xOffset[l1], animation_1.yOffset[l1], animation_1.zOffset[l1]);
        }
