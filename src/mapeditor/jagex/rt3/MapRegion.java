...
import mapeditor.jagex.rt3.model.object.ObjectInstance;
import mapeditor.misc.HistoryTile;
                            ...
                            if(tile_layer0_type > 0 || tile_layer1_type > 0 || (render_mode & RENDER_HIDDEN_BIT) != 0)
                            {
                                ...
                                if(tile_layer0_type > 0 || (render_mode & RENDER_HIDDEN_BIT) != 0)
                                {
                                    ...
                                    if (tile_layer0_type == 0 && (render_mode & RENDER_HIDDEN_BIT) != 0){
                                        ...
                                        s = 128;
                                        l = 128;
                                    } else if ((render_mode & NO_BLENDING_BIT) == 0){ 
                                    	if (tile_hue_shift != 0)//Not hidden
...
label5:
    ...
    public static int rgb_to_hsl(int rgb) {
     ...
     */
    private static int pack_hsl(int hue, int saturation, int lightness)
    {
        if(lightness > 179)
            saturation /= 2;
        if(lightness > 192)
    ...
    public void add_object(int object_id, int object_type, int object_orientation, int tile_y, int tile_x, int tile_z, TileSetting tileSetting, SceneGraph sceneGraph)
    {
	...
	public static ArrayList<MapObject> objectData = new ArrayList<MapObject>();

	
    public final void load_object_block(int block_x, int block_z, byte block_data[], TileSetting tileSettings[], SceneGraph sceneGraph, int idx)
    {
    	objectData = new ArrayList<MapObject>();
label0:
        {
            Packet stream = new Packet(block_data);
            int object_id = -1;
            do
            {
                ...
                do
                {
                    ...
                    if(object_x > 0 && object_z > 0 && object_x < size_x - 1&& object_z < size_z - 1)
                    {
                        ...
                        MapObject m = new MapObject(object_id, (Main.loadedMapX*64)+object_x, (Main.loadedMapZ*64)+object_z, (byte) object_orientation, (byte) object_type, (byte) tile_y);
                        //m.print();
                        objectData.add(m);
                        
                        add_object(object_id, object_type, object_orientation, tile_y, object_x, object_z, tileSetting, sceneGraph);
                        //EditorMain.historyTiles[EditorMain.iterator] = new HistoryTile(object_id,object_z, object_x, tile_y, 0, object_type, object_orientation);
                       // EditorMain.iterator ++;
                    }
                } while(true);
            } while(true);
                ...
                do
                {
                    ...
                    if(tile_y == subblock_y && tile_x >= subblock_x && tile_x < subblock_x + 8 && tile_z >= subblock_z && tile_z < subblock_z + 8)
                    {
                        ...
                        if(object_x > 0 && object_z > 0 && object_x < size_x - 1 && object_z < size_z - 1)
                        {
                           ...
                           // EditorMain.historyTiles[EditorMain.iterator] = new HistoryTile(object_id,object_z, object_x, block_y, 0, object_type, object_orientation + orientation & 3);
                          //  EditorMain.iterator ++;
                        }
                    }
                } while(true);
    ...
    public final byte[][][] tile_layer1_type;
    static int anInt131;
