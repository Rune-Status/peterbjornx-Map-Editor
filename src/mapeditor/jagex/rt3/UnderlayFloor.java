...
public class UnderlayFloor {

	private static final boolean WRITE_HD = true;

	public static void unpackConfig(JagexArchive jagexArchive) {
		try {
			Packet stream = new Packet(Files.readAllBytes(Paths.get("flo.dat")));
			int cacheSize = stream.g2();
			if (cache == null)
				cache = new UnderlayFloor[cacheSize];
			for (int j = 0; j < cacheSize; j++) {
				if (cache[j] == null)
					cache[j] = new UnderlayFloor();
				cache[j].id = j;
	...
	private UnderlayFloor() {
		texture = -1;
		occlude = true;
	}

	public static UnderlayFloor cache[];
	public int colour2;
	public int texture;
	public int textureC;
	...
	public UnderlayFloor cloneFLO() {
		UnderlayFloor flo2 = new UnderlayFloor();
	...
	public void replace(UnderlayFloor flo2) {
	...
	public static int addNew(UnderlayFloor floor) {
		UnderlayFloor floors[] = new UnderlayFloor[cache.length + 1];
		System.arraycopy(cache, 0, floors, 0, cache.length);
		floors[cache.length] = new UnderlayFloor();
